// FILE: 01_critical_injection.go
// SEVERITY: CRITICAL
// VULNERABILITIES COVERED:
//   [CRITICAL] SQL Injection via string concatenation
//   [CRITICAL] OS Command Injection via exec.Command with user input
//   [CRITICAL] Unsafe deserialization (gob with untrusted input)
//   [CRITICAL] Server-Side Template Injection (html/template bypassed via text/template)
...
// Invoice is the target struct for XML unmarshalling.
type Invoice struct {
...
// parseInvoiceXML parses an XML invoice from the request body.
// VULNERABLE: Go's encoding/xml does NOT expand external entities by default,
// BUT if a custom decoder or third-party lib (e.g. etree, libxml2 bindings) is used
// without disabling DTD processing, XXE is possible.
// Here we also show the pattern where the raw body is logged, leaking sensitive data.
func parseInvoiceXML(r *http.Request) (*Invoice, error) {
	body, err := io.ReadAll(r.Body)
	if err != nil {
		return nil, err
	}

	// ❌ VULN: logging raw XML body leaks PII / secrets in log files
	log.Printf("[DEBUG] Raw XML body: %s", string(body))

	var invoice Invoice
	// ❌ VULN: if a custom XML parser (e.g. github.com/beevik/etree) is used that
	// supports DOCTYPE/ENTITY, this becomes a full XXE. Shown as pattern.
	if err := xml.Unmarshal(body, &invoice); err != nil {
		return nil, fmt.Errorf("xml parse error: %w — raw: %s", err, string(body))
	}
	return &invoice, nil
}

// processXMLFeed blindly processes an external XML feed URL supplied by user.
// VULNERABLE: combines SSRF + potential XXE if the fetched document has external entities.
func processXMLFeed(feedURL string) ([]Invoice, error) {
	// ❌ VULN: SSRF — fetches arbitrary user-supplied URL (could be internal metadata endpoint)
	...
	// ❌ VULN: unmarshal response from untrusted external source without schema validation
	var invoices []Invoice
	if err := xml.Unmarshal(body, &invoices); err != nil {
		return nil, err
	}
	return invoices, nil
}
...
// ─────────────────────────────────────────────
// HTTP HANDLERS (wiring everything together)
// ─────────────────────────────────────────────
...
func handleSearch(w http.ResponseWriter, r *http.Request) {
	...
	users, err := getUserByUsername(username)
	if err != nil {
		// ❌ VULN: exposing raw DB error to client (also info disclosure)
		http.Error(w, "DB error: "+err.Error(), http.StatusInternalServerError)
		return
	}
	fmt.Fprintf(w, "Results: %+v", users)
}

func handlePing(w http.ResponseWriter, r *http.Request) {
	host := r.URL.Query().Get("host")
	out, err := pingHost(host)
	if err != nil {
...
func handleDeserialize(w http.ResponseWriter, r *http.Request) {
	payload, err := deserializeRequest(r)
	if err != nil {
		http.Error(w, err.Error(), 400)
		return
	}
	fmt.Fprintf(w, "Processed payload for user %d", payload.UserID)
}
...
func handleXML(w http.ResponseWriter, r *http.Request) {
	inv, err := parseInvoiceXML(r)
	if err != nil {
		http.Error(w, err.Error(), 400)
		return
	}
	fmt.Fprintf(w, "Invoice: %+v", inv)
}

// ─────────────────────────────────────────────
// HELPER: demonstrate second-order SQL injection
// ─────────────────────────────────────────────

// updateEmail stores an email and later uses it in a query — second-order injection.
func updateEmail(userID int, newEmail string) error {
	// ❌ VULN: storing unsanitised input and reusing it in SQL later
	_, err := db.Exec("UPDATE users SET email = '" + newEmail + "' WHERE id = " +
		fmt.Sprintf("%d", userID))
	return err
}

func getUserByEmail(email string) (*User, error) {
	// ❌ VULN: email was stored without sanitisation and is now re-injected
...
func main() {
	...
	http.HandleFunc("/deserialize", handleDeserialize)
	http.HandleFunc("/welcome", handleTemplate)
	http.HandleFunc("/invoice", handleXML)

	// ❌ VULN: binding on all interfaces with no TLS
...
// ─────────────────────────────────────────────
// UTILITY: string builder used in dynamic queries
// ─────────────────────────────────────────────

// buildFilterQuery constructs a WHERE clause from a map of filters.
// VULNERABLE: all values are directly interpolated.
func buildFilterQuery(table string, filters map[string]string) string {
	var sb strings.Builder
	sb.WriteString(fmt.Sprintf("SELECT * FROM %s WHERE ", table))
	i := 0
	for k, v := range filters {
		if i > 0 {
			sb.WriteString(" AND ")
		}
		// ❌ VULN: both key and value are attacker-controlled
...
// executeRawQuery runs any SQL string — intentionally unrestricted admin backdoor.
// VULNERABLE: this function should never exist in production code.
func executeRawQuery(w http.ResponseWriter, r *http.Request) {
	rawSQL := r.FormValue("sql")
	// ❌ VULN: executing arbitrary SQL from HTTP parameter
